...
ExamineR <- function() {
  ...
  for (exam.count in 1:number.of.exam.files) {
    ...
    setWinProgressBar(progress.bar, exam.count, title = paste(progress.bar.title,
...
#  path.to.examiner.folder <- choose.dir()
  ...
  for (i in 1:number.of.students) {
    ...
    setWinProgressBar(progress.bar, i, title = paste(progress.bar.title, round(i / number.of.students *
                                                             100, 0),
                                                     "% done", sep = ""))
  }
  close(progress.bar)
